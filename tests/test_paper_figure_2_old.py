...
def test_build_output_path_uses_requested_format() -> None:
    assert build_output_path(
        Path("paper_figures"), "figure_2_old", "svg"
    ) == Path(
        "paper_figures/figure_2_old.svg"
    )

    with pytest.raises(ValueError, match="Unknown output format"):
        build_output_path(Path("paper_figures"), "figure_2_old", "jpg")
...
def test_default_cli_matches_shared_figure_3_canvas() -> None:
    ...
    assert DEFAULT_OUTPUT_NAME == "figure_2_old"
...
def test_make_figure_2_splits_decoding_panel_and_swaps_c_d_locations(
    ...
    output_path = tmp_path / "figure_2_old.svg"
