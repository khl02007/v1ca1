"""Tests for the three-model Figure 3_2 variant."""
...
from v1ca1.paper_figures import figure_3_2 as figure_3_2_module
...
def _make_panel_b_example(example_index: int) -> dict[str, object]:
    """Return one synthetic example containing all Figure 3_2 traces."""
    ...
    return {
        ...
        "models": {
            ...
            figure_3_2_module.PANEL_B_ADDITIVE_MODEL_NAME: [
                additive_offset,
                additive_offset + 1.0,
                additive_offset,
            ],
...
def test_defaults_and_output_path_match_figure_3_conventions() -> None:
    args = figure_3_2_module.parse_arguments([])

    assert figure_3_2_module.DEFAULT_OUTPUT_NAME == "figure_3_2"
    assert args.output_name == "figure_3_2"
    assert args.output_dir == figure_3_module.DEFAULT_OUTPUT_DIR
    assert args.output_format == figure_3_module.DEFAULT_OUTPUT_FORMAT
    assert figure_3_2_module.DEFAULT_FIGURE_WIDTH_MM == pytest.approx(
        figure_3_module.DEFAULT_FIGURE_WIDTH_MM
    )
    assert figure_3_2_module.DEFAULT_FIGURE_HEIGHT_MM == pytest.approx(
        figure_3_2_module.TOP_ROW_HEIGHT_MM
        + figure_3_2_module.PANEL_B_ROW_HEIGHT_MM
        + figure_3_2_module.PANEL_C_ROW_HEIGHT_MM
    )
    assert figure_3_2_module.PANEL_B_ROW_HEIGHT_MM == pytest.approx(
        0.9 * figure_3_module.BOTTOM_ROW_HEIGHT_MM
    )
    assert figure_3_2_module.PANEL_C_ROW_HEIGHT_MM == pytest.approx(
        0.9 * figure_3_module.BOTTOM_ROW_HEIGHT_MM
    )
    assert figure_3_2_module.PANEL_TITLES == (
        "Three models that relate dark and light activity",
        figure_3_module.PANEL_TITLES[1],
    )
    ...
    assert (
        figure_3_2_module.PANEL_C_MODEL_LABELS[comparison_model]
        == "Multiplicative"
    )
    assert (
        figure_3_2_module.PANEL_B_MODEL_LABELS[
            figure_3_2_module.PANEL_B_ADDITIVE_MODEL_NAME
        ]
        == "Additive"
    )
    assert figure_3_2_module.PANEL_B_SWAP_EXAMPLES == (
    ...
    assert (
        figure_3_2_module.PANEL_B_SWAP_EXAMPLES
        != figure_2_module.PANEL_C_SWAP_EXAMPLES
    )
    assert figure_3_2_module.PANEL_C_HISTOGRAM_TITLES == (
        "Multiplicative vs. independent",
        "Multiplicative vs. additive",
    )
    independent_comparison_bounds = (
        figure_3_2_module.PANEL_C_MULTIPLICATIVE_VS_INDEPENDENT_HISTOGRAM_AXIS_BOUNDS
    )
    additive_comparison_bounds = (
        figure_3_2_module.PANEL_C_MULTIPLICATIVE_VS_ADDITIVE_HISTOGRAM_AXIS_BOUNDS
    )
    assert (
    ...
    assert "Dark scaffold" not in (
        figure_3_2_module.PANEL_B_MODEL_LABELS.values()
    )
    assert "Dark scaffold" not in (
        figure_3_2_module.PANEL_C_MODEL_LABELS.values()
    )
    assert (
        figure_2_module.PANEL_C_SWAP_MODEL_LABELS_2_3[comparison_model]
        == "Dark scaffold"
    )
    assert figure_3_2_module.build_output_path(
        args.output_dir,
        args.output_name,
        "svg",
    ) == Path("paper_figures/output/figure_3_2.svg")
...
def test_panel_data_loader_merges_additive_predictions_by_example_identity(
    ...
    comparison_model = figure_2_module.PANEL_C_SWAP_MODEL_NAME
    additive_model = figure_3_2_module.PANEL_B_ADDITIVE_MODEL_NAME
    requested_examples = figure_3_2_module.PANEL_B_SWAP_EXAMPLES
    swap_delta = object()
    ...
    def fake_load_additive_delta(**kwargs: object) -> object:
        calls["additive_delta_kwargs"] = kwargs
    ...
    monkeypatch.setattr(
        ...
        fake_load_additive_delta,
    )
    result = figure_3_2_module.load_figure_3_2_panel_data(
    ...
    assert [
        figure_3_2_module._swap_example_key(example)
    ...
    assert calls["additive_delta_kwargs"] == {
        "data_root": Path("/analysis"),
        "datasets": (("L14", "20240611", "08_r4"),),
        "region": "v1",
        "dark_epoch": "08_r4",
        "light_epoch_pairs": (
            (
                figure_3_2_module.PANEL_H_TRAIN_LIGHT_EPOCH,
                figure_3_2_module.PANEL_H_HELDOUT_LIGHT_EPOCH,
            ),
        ),
...
def test_panel_a_additive_component_completes_the_light_field(
    ...
    def record_rate_gain(ax: Any, gain: float) -> None:
        ax._figure_3_2_rate_gain = float(gain)
    ...
    fig, ax = plt.subplots()
    figure_3_2_module.plot_panel_a_three_model_architecture(ax)
    fig.canvas.draw()
    ...
    assert _relative_bounds(
        ...
        rate_colorbar_axis,
    ) == pytest.approx(figure_3_2_module.PANEL_A_RATE_SCALE_BOUNDS)
    assert (
        figure_3_2_module.PANEL_A_RATE_SCALE_BOUNDS[2]
        < figure_2_module.PANEL_D2_RATE_COLORBAR_BOUNDS[2]
    )
    assert (
        figure_3_2_module.PANEL_A_RATE_SCALE_BOUNDS[3]
        < figure_2_module.PANEL_D2_RATE_COLORBAR_BOUNDS[3]
    )
    assert rate_colorbar_axis.get_title() == (
        figure_3_2_module.PANEL_A_RATE_SCALE_LABEL
    )
    assert figure_3_2_module.PANEL_A_RATE_SCALE_LABEL == "FR"
    assert rate_colorbar_axis.title.get_color() == (
        figure_3_2_module.PANEL_A_RATE_SCALE_COLOR
    )
    assert not any(
        tick_label.get_text()
    ...
    assert rate_colorbar_axis.spines["outline"].get_edgecolor() == (
        pytest.approx(
            to_rgba(figure_3_2_module.PANEL_A_RATE_SCALE_OUTLINE_COLOR)
        )
    )
    assert rate_colorbar_axis.spines["outline"].get_linewidth() == (
        pytest.approx(
            figure_3_2_module.PANEL_A_RATE_SCALE_OUTLINE_LINEWIDTH
        )
    )
    ...
    component_gain = figure_3_2_module.PANEL_A_ADDITIVE_FIELD_RATE_GAIN
    assert component_gain == pytest.approx(light_gain - dark_gain)
    assert dark_gain + component_gain == pytest.approx(light_gain)
    assert additive_axes[0]._figure_3_2_rate_gain == pytest.approx(dark_gain)
    assert additive_component_ax._figure_3_2_rate_gain == pytest.approx(
        component_gain
    )
    assert additive_light_ax._figure_3_2_rate_gain == pytest.approx(light_gain)
    ...
    assert figure_3_2_module.ADDITIVE_COMPONENT_LABEL in visible_text_values
    assert visible_text_values.count(
        figure_3_2_module.PANEL_A_STIMULUS_SWAP_PREDICTION_LABEL
    ) == 1
    ...
    operator_rows = {
        row_y: sorted(
            ...
            for text in visible_texts
            if text.get_text()
            in {"+", figure_3_2_module.PANEL_A_MULTIPLICATION_SYMBOL, "="}
    ...
    assert operator_rows == {
        multiplicative_y: [
            "=",
            figure_3_2_module.PANEL_A_MULTIPLICATION_SYMBOL,
        ],
        additive_y: ["+", "="],
    }
    plt.close(fig)
...
def test_panel_b_swap_examples_span_the_full_row(
    ...
    examples = [
        ...
        for index in range(len(figure_3_2_module.PANEL_B_SWAP_EXAMPLES))
    ]
    model_colors = figure_3_2_module.PANEL_B_MODEL_COLORS
    model_labels = figure_3_2_module.PANEL_B_MODEL_LABELS
    fig, ax = plt.subplots()
    figure_3_2_module.plot_panel_b_swap_examples_panel(
        ax,
        examples,
        model_name=figure_2_module.PANEL_C_SWAP_MODEL_NAME,
    ...
    for example_axis, expected_bounds in zip(
        example_axes,
        figure_3_2_module.PANEL_B_EXAMPLE_SLOT_BOUNDS,
        strict=True,
    ):
    ...
    for kwargs in example_kwargs:
        ...
        assert kwargs["icon_bounds"] == (
            figure_3_2_module.PANEL_B_EXAMPLE_ICON_BOUNDS
        )
    ...
    shared_xlabels = [
        ...
        if text.get_text() == figure_3_2_module.PANEL_B_SHARED_XLABEL
    ]
    assert figure_3_2_module.PANEL_B_SHARED_XLABEL == (
        "Norm. path progression (switched segment only)"
    )
    assert len(shared_xlabels) == 1
    assert shared_xlabels[0].get_position() == pytest.approx(
        figure_3_2_module.PANEL_B_SHARED_XLABEL_POSITION
    )
    ...
    legend_axes = [
        ...
        == pytest.approx(figure_3_2_module.PANEL_B_TRACE_LEGEND_SLOT_BOUNDS)
    ]
    ...
    assert _relative_bounds(ax, legend_axis) == pytest.approx(
        figure_3_2_module.PANEL_B_TRACE_LEGEND_SLOT_BOUNDS
    )
...
def test_panel_b_plots_the_corresponding_additive_prediction_for_each_example(
    ...
    example_axes = sorted(
        (
            ...
            if any(
                ...
                for bounds in figure_3_2_module.PANEL_B_EXAMPLE_SLOT_BOUNDS
            )
        ),
    ...
    model_names_by_label = {
        ...
        "Additive": figure_3_2_module.PANEL_B_ADDITIVE_MODEL_NAME,
    }
    ...
    for example_index, (example_axis, example) in enumerate(
        ...
        assert empirical_line.get_linewidth() == pytest.approx(
            figure_3_2_module.PANEL_B_EMPIRICAL_LINEWIDTH
        )
        ...
        for trace_label, model_name in model_names_by_label.items():
            ...
            expected_style = Line2D(
                ...
                [],
                linestyle=(
                    figure_3_2_module.PANEL_B_MODEL_LINESTYLES[model_name]
                ),
            )
...
def test_panel_c_delta_tables_use_the_same_matched_forward_heldout_cohort() -> None:
    ...
    paired_independent, paired_additive = (
        figure_3_2_module._build_panel_c_delta_tables(
...
def test_panel_c_compares_multiplicative_with_independent_then_additive(
        ...
        [
            {
                **identity_rows[1],
                "trajectory": trajectory,
                "model_name": figure_3_2_module.PANEL_B_ADDITIVE_MODEL_NAME,
    ...
    model_labels = figure_3_2_module.PANEL_C_MODEL_LABELS
    fig, ax = plt.subplots()
    figure_3_2_module.plot_panel_c_swap_histogram_panel(
        ax,
        multiplicative_delta,
        additive_delta,
        model_name=figure_2_module.PANEL_C_SWAP_MODEL_NAME,
    ...
    assert to_rgba(additive_kwargs["model_colors"]["visual"]) == pytest.approx(
        to_rgba(figure_3_2_module.PANEL_B_ADDITIVE_MODEL_COLOR)
    )
    ...
    assert independent_axis.get_title() == (
        figure_3_2_module.PANEL_C_HISTOGRAM_TITLES[0]
    )
    assert additive_axis.get_title() == (
        figure_3_2_module.PANEL_C_HISTOGRAM_TITLES[1]
    )
    ...
    shared_xlabels = [
        text
        for text in ax.texts
        if text.get_text() == figure_3_2_module.PANEL_C_SHARED_XLABEL
    ]
    assert len(shared_xlabels) == 1
    assert figure_3_2_module.PANEL_C_SHARED_XLABEL == (
        "Δ log likelihood (bits/spike)"
    )
    assert "\n" not in shared_xlabels[0].get_text()
    assert shared_xlabels[0].get_transform() is ax.transAxes
    assert shared_xlabels[0].get_position() == pytest.approx(
        figure_3_2_module.PANEL_C_SHARED_XLABEL_POSITION
    )
    for histogram_axis in (independent_axis, additive_axis):
        ...
        assert count_labels[0].get_position()[1] == pytest.approx(
            figure_3_2_module.PANEL_C_BOTTOM_ANNOTATION_Y
        )
...
def test_panel_c_shared_xlabel_matches_panel_b_tick_label_gap(
    ...
    original_plot_panel_b = figure_3_2_module.plot_panel_b_swap_examples_panel
    original_plot_panel_c = figure_3_2_module.plot_panel_c_swap_histogram_panel
    ...
    def fake_save_figure(
        ...
        **kwargs: object,
        ...
        calls["panel_b_gap_points"] = _shared_xlabel_tick_gap_points(
            fig,
            calls["panel_b_axis"],
            figure_3_2_module.PANEL_B_SHARED_XLABEL,
        )
        calls["panel_c_gap_points"] = _shared_xlabel_tick_gap_points(
            fig,
            calls["panel_c_axis"],
            figure_3_2_module.PANEL_C_SHARED_XLABEL,
        )
        return output_path

    monkeypatch.setattr(
        figure_3_2_module,
        "load_figure_3_2_panel_data",
        fake_load_panel_data,
    )
    monkeypatch.setattr(
        figure_3_2_module,
    ...
    figure_3_2_module.make_figure_3_2(
        data_root=Path("/analysis"),
...
def test_make_figure_3_2_uses_three_full_width_rows_and_split_swap_panels(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    matplotlib = pytest.importorskip("matplotlib")
    ...
    def fail_if_old_architecture_is_plotted(*args: object, **kwargs: object) -> None:
        del args, kwargs
        raise AssertionError("Figure 3_2 used the two-model architecture renderer")
    ...
    output_path = tmp_path / "figure_3_2.svg"
    saved_path = figure_3_2_module.make_figure_3_2(
        data_root=Path("/analysis"),
    ...
    assert calls["figsize"] == pytest.approx(
        (
            figure_3_2_module.DEFAULT_FIGURE_WIDTH_MM / 25.4,
            figure_3_2_module.DEFAULT_FIGURE_HEIGHT_MM / 25.4,
        )
    )
    assert calls["titles"] == figure_3_2_module.PANEL_TITLES
    assert calls["labels"] == {"A", "B", "C"}
    assert set(calls["label_font_sizes"].values()) == {
        figure_3_2_module.PANEL_LABEL_FONTSIZE
    }
    assert calls["label_positions"]["A"] == pytest.approx(
        (
            figure_3_2_module.PANEL_A_B_LABEL_X,
            figure_2_module.PANEL_B_LABEL_Y,
        )
    )
    assert calls["label_positions"]["B"] == pytest.approx(
        (
            figure_3_2_module.PANEL_B_LABEL_X,
            figure_3_2_module.PANEL_ROW_LABEL_Y,
        )
    )
    assert calls["label_positions"]["C"] == pytest.approx(
        (
            figure_3_2_module.PANEL_C_LABEL_X,
            figure_3_2_module.PANEL_ROW_LABEL_Y,
        )
    )
...
def test_main_builds_figure_3_2_output_and_forwards_cli_options(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    calls: dict[str, object] = {}

    def fake_make_figure_3_2(**kwargs: object) -> Path:
        calls.update(kwargs)
    ...
    monkeypatch.setattr(
        figure_3_2_module,
        "make_figure_3_2",
        fake_make_figure_3_2,
    )
    figure_3_2_module.main(
        [
            "--data-root",
            "/analysis",
            "--output-dir",
