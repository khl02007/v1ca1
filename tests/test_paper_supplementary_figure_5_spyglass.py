"""Tests for the database-free Supplementary Figure 5 adapter."""
...
from v1ca1.paper_figures import supplementary_figure_5_spyglass as figure
...
def _payload(run_dir: Path) -> dict[str, Any]:
    """Return one minimal validated Supplementary Figure 5 payload."""
    return {
        "run_dir": run_dir,
        "campaign": {
            "run_id": run_dir.name,
...
def test_adapter_targets_supplementary_figure_5_and_cli_defaults() -> None:
    assert figure.canonical.DEFAULT_OUTPUT_NAME == "supplementary_figure_5"
    assert figure.DEFAULT_OUTPUT_NAME == "supplementary_figure_5_spyglass"
...
def test_payload_loads_only_modulation_and_source_comparison_inputs(
    ...
    payload = figure.load_supplementary_figure_5_payload(
        run_id="figure3-nwb-gpu-v2",
...
def test_render_is_atomic_run_local_receipted_and_promotable(
    ...
    def make_figure(**kwargs: Any) -> Path:
        ...
        output_path.write_text("supplementary figure 5", encoding="utf-8")
        return output_path
    ...
    returned = figure.render_supplementary_figure_5(
        payload,
    ...
    assert output_path.read_text(encoding="utf-8") == "supplementary figure 5"
    ...
    with pytest.raises(FileExistsError, match="Refusing to overwrite"):
        figure.render_supplementary_figure_5(
            payload,
            output_path=output_path,
        )
    ...
    published = publication_dir / "supplementary_figure_5.svg"
    assert figure.promote_supplementary_figure_5(
        payload,
