...
import v1ca1.ripple.ripple_glm2 as ripple_glm2_module
...
def test_load_modern_ripple_tables_groups_rows_by_epoch(tmp_path: Path) -> None:
    analysis_path = tmp_path / "RatA" / "20240101"
    _write_ripple_parquet(
        analysis_path,
        rows=[
            {"epoch": "01_s1", "start": 1.0, "end": 1.2, "mean_zscore": 2.0},
            {"epoch": "01_s1", "start": 2.5, "end": 2.7, "mean_zscore": 3.0},
            {"epoch": "02_r1", "start": 5.0, "end": 5.2, "mean_zscore": 4.0},
        ],
    )

    ripple_tables = ripple_glm2_module.load_modern_ripple_tables(analysis_path)
...
def test_prepare_ripple_glm2_session_rejects_missing_modern_inputs_even_if_pickles_exist(
    tmp_path: Path,
) -> None:
    analysis_path = tmp_path / "RatA" / "20240101"
    analysis_path.mkdir(parents=True)
    with open(analysis_path / "timestamps_ephys.pkl", "wb") as file:
        pickle.dump({"01_s1": np.array([0.0, 1.0], dtype=float)}, file)
    with open(analysis_path / "timestamps_ephys_all.pkl", "wb") as file:
        pickle.dump(np.array([0.0, 1.0], dtype=float), file)
    ripple_dir = analysis_path / "ripple"
    ripple_dir.mkdir()
    with open(ripple_dir / "Kay_ripple_detector.pkl", "wb") as file:
        pickle.dump({"01_s1": {"start_time": [0.2], "end_time": [0.3]}}, file)

    with pytest.raises(FileNotFoundError, match="timestamps_ephys.npz"):
        ripple_glm2_module.prepare_ripple_glm2_session(
            animal_name="RatA",
            date="20240101",
            data_root=tmp_path,
        )
...
def test_prepare_ripple_glm2_session_uses_modern_loaders(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    analysis_path = tmp_path / "RatA" / "20240101"
    timestamps_ephys_all = _write_ephys_npz(
        analysis_path,
        epoch_tags=["01_s1", "02_r1"],
        epoch_segments=[
            np.linspace(0.0, 0.9, 10, dtype=float),
            np.linspace(2.0, 2.9, 10, dtype=float),
        ],
    )
    _write_ripple_parquet(
        analysis_path,
        rows=[
            {"epoch": "01_s1", "start": 0.2, "end": 0.3},
            {"epoch": "02_r1", "start": 2.2, "end": 2.3},
        ],
    )
    ...
    monkeypatch.setattr(ripple_glm2_module, "load_spikes_by_region", _fake_load_spikes_by_region)

    session = ripple_glm2_module.prepare_ripple_glm2_session(
        animal_name="RatA",
        date="20240101",
        data_root=tmp_path,
    )

    assert session["sources"]["timestamps_ephys"] == "pynapple"
    assert session["sources"]["timestamps_ephys_all"] == "pynapple"
    assert session["sources"]["ripple_events"] == "parquet"
...
def test_prepare_ripple_glm2_session_warns_when_legacy_ripple_pickle_differs(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    analysis_path = tmp_path / "RatA" / "20240101"
    ...
    _write_legacy_ripple_pickle(
        analysis_path,
        tables={"01_s1": {"start_time": [0.2, 0.5], "end_time": [0.3, 0.6]}},
    )
    (analysis_path / "sorting_v1").mkdir()
    (analysis_path / "sorting_ca1").mkdir()
    ...
    with pytest.warns(UserWarning, match="does not match the legacy"):
        ripple_glm2_module.prepare_ripple_glm2_session(
            animal_name="RatA",
            date="20240101",
            data_root=tmp_path,
...
def test_save_results_npz_round_trips_legacy_keys(tmp_path: Path) -> None:
    output_path = tmp_path / "result.npz"
    results = _fake_fit_results()

    ripple_glm2_module.save_results_npz(output_path, results)
    loaded = ripple_glm2_module.load_results_npz(output_path)

    assert output_path.exists()
    assert "yhat_ripple" in loaded
    assert "ll_ripple_folds" in loaded
    assert "y_pre_test" in loaded
    assert "yhat_pre" in loaded
    assert "ll_pre" in loaded
    assert isinstance(loaded["fold_info"], list)
    assert np.allclose(loaded["pseudo_r2_pre"], [0.05, 0.06])


def test_build_legacy_result_path_matches_legacy_name(tmp_path: Path) -> None:
    result_path = ripple_glm2_module.build_legacy_result_path(
        tmp_path,
        epoch="02_r1",
        exclude_ripples=False,
        ripple_window_s=0.2,
    ...
    assert (
        result_path.name
        == "02_r1_train_ripple_predict_ripple_and_pre_exclude_ripples_False_ripple_window_0.2_min_spikes_per_ripple_0.1_ridge_strength_0.1.npz"
    )


def test_main_smoke_writes_legacy_npz_and_figures(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    analysis_path = tmp_path / "RatA" / "20240101"
    ...
    monkeypatch.setattr(ripple_glm2_module, "plot_ripple_glm_pseudo_r2", _touch_plot)
    monkeypatch.setattr(ripple_glm2_module, "plot_ripple_glm_mae", _touch_plot)
    monkeypatch.setattr(ripple_glm2_module, "plot_ripple_glm_delta_ll", _touch_plot)
    monkeypatch.setattr(ripple_glm2_module, "plot_pre_pseudo_r2_summary", _touch_plot)
    monkeypatch.setattr(ripple_glm2_module, "plot_pre_mae_summary", _touch_plot)
    monkeypatch.setattr(ripple_glm2_module, "plot_pre_ll_summary", _touch_plot)

    ripple_glm2_module.main(
        [
            "--animal-name",
            "RatA",
            "--date",
    ...
    loaded = ripple_glm2_module.load_results_npz(result_path)
    ...
    figure_paths = list(figure_dir.glob("*.png"))
