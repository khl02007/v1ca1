...
def fit_headdir_decode_cv(
    ...
    print(
        f"Fitting head direction decoder "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
        f"n_folds {n_folds}, "
    ...
    head_direction_deg_unwrapped_interp = f_head_direction_unwrapped(time)
    ...
    environment = rtc.Environment(
        place_bin_size=headdir_bin_size,
    )
    ...
    for region in regions:
        ...
        for fold_idx in range(n_folds):
            ...
            # fit classifiers
            classifier.fit(
                (head_direction_deg_unwrapped_interp[is_training] + 180) % (2 * 180)
                - 180,
                spike_indicator[is_training],
...
def parse_arguments():
    ...
    parser.add_argument(
        "--n_folds",
        type=int,
        help="Number of folds for doing cross validation",
    )
    parser.add_argument(
        "--movement",
        type=str,
