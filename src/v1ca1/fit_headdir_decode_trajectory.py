...
# fit
def fit_headdir_decode_trajectory(
    epoch_idx,
    use_half="all",
    movement="True",
    headdir_std=10,
    ...
    print(
        f"Fitting head direction decoder by trajectory type "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
    ...
    f_pos_body = scipy.interpolate.interp1d(
        t_position, position_body, axis=0, bounds_error=False, kind="linear"
    )
        ...
        for trajectory_type in trajectory_types:
            ...
            for traj_start_time, traj_stop_time in trajectory_times[epoch][
                ...
                if inds_to_keep.sum() == 0:
                    continue
                traj_headdir.append(head_direction_interp[inds_to_keep])
                spike_indicator_traj.append(spike_indicator[region][inds_to_keep])

            traj_headdir = np.concatenate(traj_headdir, axis=0)
...
def main():
    ...
    fit_headdir_decode_trajectory(
        args.epoch_idx,
        args.use_half,
        args.movement,
        args.headdir_std,
        args.discrete_var,
        args.headdir_bin_size,
        args.movement_var,
        args.overwrite,
    )
