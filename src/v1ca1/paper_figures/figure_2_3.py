"""Generate a visually streamlined alternate Figure 2."""
...
DEFAULT_OUTPUT_NAME = "figure_2_3"
...
FIGURE_2_3_CONSTRAINED_LAYOUT_PADS = {
    **FIGURE_2_CONSTRAINED_LAYOUT_PADS,
    "hspace": 0.08,
}
...
def plot_panel_b_dpp_overlap_with_schematic(
    ...
    *,
    ...
    """Plot Figure 2_3 Panel B with DPPI schematic and overlap summaries."""
    ax.set_xlim(0.0, 1.0)
    ax.set_ylim(0.0, 1.0)
    ax.axis("off")
...
def plot_panel_c_cross_and_place_decoding(
    ...
    """Plot Figure 2_3 Panel C as compact cross-route and place decoding."""
    ax.set_xlim(0.0, 1.0)
    ax.set_ylim(0.0, 1.0)
    ax.axis("off")
...
def plot_panel_d_compact_swap_delta(
    ...
    *,
    ...
    """Plot Figure 2_3 Panel D with three examples and one mean histogram."""
    ax.set_xlim(0.0, 1.0)
    ax.set_ylim(0.0, 1.0)
    ax.axis("off")
...
def plot_panel_c_model_architecture_row(
    ...
    """Plot Figure 2_3 Panel C with schematics left and examples right."""
    ax.set_xlim(0.0, 1.0)
    ax.set_ylim(0.0, 1.0)
    ax.axis("off")
...
def make_figure_2_3(
    *,
    ...
    """Build and save the streamlined alternate Figure 2."""
    ...
    fig.get_layout_engine().set(**FIGURE_2_3_CONSTRAINED_LAYOUT_PADS)
    ...
    save_bbox = _tight_bbox_with_bottom_crop(fig, FIGURE_2_3_BOTTOM_CROP_MM)
    save_figure(fig, output_path, dpi=dpi, bbox_inches=save_bbox)
    plt.close(fig)
    print(f"Saved Figure 2_3 to {output_path}")
    return output_path


def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse command-line arguments for streamlined Figure 2 generation."""
    parser = argparse.ArgumentParser(
        description="Generate streamlined alternate Figure 2."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> None:
    """Run streamlined alternate Figure 2 generation."""
