"""Render Supplementary Figure 7 from a retained offline Figure 3 campaign."""
...
from v1ca1.paper_figures import supplementary_figure_7 as canonical
...
DEFAULT_OUTPUT_NAME = "supplementary_figure_7_spyglass"
...
FIGURE_ARTIFACT_KIND = "complete_spyglass_supplementary_figure_7"
...
def load_supplementary_figure_7_payload(
    *,
    run_id: str,
    scratch_root: Path = DEFAULT_SCRATCH_ROOT,
) -> dict[str, Any]:
    """Load only the retained artifacts used by Supplementary Figure 7."""
        ...
        **kwargs: Any,
        ...
        if kwargs.get("ripple_threshold_zscore") is not None:
            raise _UnexpectedLegacyRequest(
                "Supplementary Figure 7 cannot rethreshold retained ripples."
            )
    ...
    def load_source_comparison(
        ...
        **kwargs: Any,
        ...
        if tuple(kwargs.get("epoch_types", ())) != tuple(
            ...
            raise _UnexpectedLegacyRequest(
                "Supplementary Figure 7 requested foreign GLM epochs."
            )
...
def get_output_path(
    *,
    ...
    """Return the canonical run-local Supplementary Figure 7 output path."""
...
def promote_supplementary_figure_7(
    payload: Mapping[str, Any],
    *,
    source_path: Path,
    ...
    """Publish a validated Supplementary Figure 7 and its receipt."""
...
def render_supplementary_figure_7(
    payload: Mapping[str, Any],
    *,
    output_path: Path,
    dpi: int = DEFAULT_DPI,
) -> Path:
    """Atomically render Supplementary Figure 7 inside its campaign."""
    ...
    if not output_path.is_relative_to(run_dir):
        raise ValueError(
            "Supplementary Figure 7 output must remain inside its campaign run."
        )
    if output_path.exists():
        raise FileExistsError(
            "Refusing to overwrite Supplementary Figure 7 output: "
            f"{output_path}"
        )
    provenance_path = get_figure_provenance_path(output_path)
    if provenance_path.exists():
        raise FileExistsError(
            "Refusing to overwrite Supplementary Figure 7 provenance: "
            f"{provenance_path}"
        )
    if output_path.suffix.lower().lstrip(".") not in canonical.FIGURE_FORMATS:
        raise ValueError("Supplementary Figure 7 output has an unsupported format.")
    ...
    try:
        with _offline_sources(payload):
            rendered = canonical.make_supplementary_figure_7(
                data_root=run_dir,
        ...
        if Path(rendered).resolve(strict=True) != temporary_path:
            raise ValueError(
                "Supplementary Figure 7 renderer returned an unexpected path."
            )
...
def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse database-free Supplementary Figure 7 renderer arguments."""
...
def main(argv: Sequence[str] | None = None) -> None:
    """Load one Figure 3 campaign and render Supplementary Figure 7."""
    args = parse_arguments(argv)
    payload = load_supplementary_figure_7_payload(
        run_id=args.run_id,
    ...
    path = render_supplementary_figure_7(
        payload,
        output_path=output_path,
        dpi=args.dpi,
    )
    print(f"Saved offline Spyglass Supplementary Figure 7 to {path}")
    if args.promote_to is not None:
        promoted = promote_supplementary_figure_7(
            payload,
            source_path=path,
            destination_path=args.promote_to,
            replace=args.replace_promoted_output,
        )
        print(f"Promoted validated Supplementary Figure 7 to {promoted}")
...
__all__ = [
    ...
    "get_output_path",
    "load_supplementary_figure_7_payload",
    "main",
    "promote_supplementary_figure_7",
    "render_supplementary_figure_7",
]
