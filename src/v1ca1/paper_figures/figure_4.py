...
"""Generate Figure 4 panels for CA1 ripple modulation of V1 activity."""
...
DEFAULT_FIGURE_4_GLM_RIPPLE_SELECTION = "single"
...
def load_pooled_ripple_heatmap_epoch_tables(
    ...
    *,
    ...
    for dataset in datasets:
        ...
        epoch_ids = make_figure_4_epoch_ids(
            animal_name,
            date,
...
def _format_figure_cache_token(value: Any) -> str:
    """Return a filesystem-safe token for one Figure 4 cache value."""
    token = "".join(
    ...
    for dataset_id in datasets:
        ...
        for epoch_type in selected_epoch_types:
            if epoch_type not in epoch_ids:
                raise ValueError(f"Unknown Figure 4 epoch type: {epoch_type!r}")
            _glm_animal, _glm_date, glm_epoch = normalize_dataset_id(epoch_ids[epoch_type])
            glm_path = get_ripple_glm_path(
                data_root,
                animal_name=animal_name,
...
def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate Figure 4 CA1 ripple modulation and CA1-to-V1 GLM panels."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=DEFAULT_FIGURE_4_GLM_RIPPLE_SELECTION,
        help=(
            "Ripple-GLM selection suffix for Figure 4 Panels C and D. "
            f"Default: {DEFAULT_FIGURE_4_GLM_RIPPLE_SELECTION}"
        ),
    )
    parser.add_argument(
        "--ridge-strength",
