...
PANEL_A2_SCHEMATIC_AXIS_WIDTH = 0.070
PANEL_A2_SCHEMATIC_AXIS_HEIGHT = 0.075
PANEL_A2_MULTIROW_COLUMN_GAP = 0.045
PANEL_A2_MULTIROW_ROW_GAP = 0.050
...
def plot_panel_a2_examples_single_row(
    ...
    *,
    y_max_overrides: Mapping[int, float] | None = None,
    schematic_scale: float = 1.0,
    ylabel_x: float | None = None,
) -> None:
    """Plot Figure 2 Panel A in rows of at most four example cells."""
    ax.set_xlim(0.0, 1.0)
    ax.set_ylim(0.0, 1.0)
    ax.axis("off")
    if not examples:
        ...
        return
    if y_max_overrides is None:
        y_max_overrides = _figure_2.PANEL_A_EXAMPLE_Y_MAX_OVERRIDES
    schematic_scale = float(schematic_scale)
    if not np.isfinite(schematic_scale) or schematic_scale <= 0.0:
        raise ValueError("schematic_scale must be finite and positive.")
    schematic_width = PANEL_A2_SCHEMATIC_AXIS_WIDTH * schematic_scale
    schematic_height = PANEL_A2_SCHEMATIC_AXIS_HEIGHT * schematic_scale
    schematic_left = PANEL_A2_SINGLE_ROW_SCHEMATIC_AXIS_LEFT - (
        schematic_width - PANEL_A2_SCHEMATIC_AXIS_WIDTH
    )

    column_count = min(len(examples), 4)
    row_count = int(np.ceil(len(examples) / column_count))
    is_multirow = row_count > 1
    column_gap = (
        PANEL_A2_MULTIROW_COLUMN_GAP
        if is_multirow
        else _figure_2.PANEL_A_SINGLE_ROW_COLUMN_GAP
    )
    row_gap = PANEL_A2_MULTIROW_ROW_GAP if is_multirow else 0.0
    ...
    for example_index, example in enumerate(examples, start=1):
        row_index, column_index = divmod(example_index - 1, column_count)
        left, column_width = column_bounds[column_index]
        bottom = (
            1.0
        ...
        example_ax = ax.inset_axes(
            [left, bottom, column_width, row_height]
        )
        plot_kwargs: dict[str, Any] = {
            ...
            "schematic_axis_left": schematic_left,
            "schematic_axis_width": schematic_width,
            "schematic_axis_height": schematic_height,
            "show_correlation": False,
            "similarity_annotation": "dppi",
        }
        y_max_override = y_max_overrides.get(example_index)
        ...
        if ylabel_x is not None:
            for child_ax in example_ax.child_axes:
                if child_ax.get_ylabel() in {"Trials", "FR (Hz)"}:
                    child_ax.yaxis.set_label_coords(
                        float(ylabel_x),
                        0.5,
                    )
        rate_axes = [
        ...
        example_ax.text(
            ...
            fontweight="semibold" if is_multirow else "normal",
