"""Render Supplementary Figure 4 from retained Spyglass artifacts."""
...
DEFAULT_OUTPUT_NAME = "supplementary_figure_4_spyglass"
...
def load_supplementary_figure_4_payload(
    *,
    ...
    """Load every active Supplementary Figure 4 input."""
...
def _require_request(
    *,
    ...
    if Path(data_root).resolve(strict=True) != Path(payload["run_dir"]).resolve(
        ...
        raise ValueError("Supplementary Figure 4 requested a foreign root.")
...
def get_output_path(
    *,
    ...
    """Return the immutable run-local Supplementary Figure 4 path."""
...
def promote_supplementary_figure_4(
    payload: Mapping[str, Any],
    *,
    source_path: Path,
    ...
    """Publish one validated Supplementary Figure 4 and receipt."""
...
def render_supplementary_figure_4(
    payload: Mapping[str, Any],
    *,
    output_path: Path,
    dpi: int = DEFAULT_DPI,
) -> Path:
    """Atomically render Supplementary Figure 4 inside its campaign."""
    ...
    if output_path.exists() or provenance_path.exists():
        raise FileExistsError(
            f"Refusing to overwrite Supplementary Figure 4: {output_path}"
        )
    if output_path.suffix.lower().lstrip(".") not in canonical.FIGURE_FORMATS:
        raise ValueError("Unsupported Supplementary Figure 4 output format.")
    ...
    try:
        with _offline_sources(payload):
            rendered = canonical.make_supplementary_figure_4(
                data_root=run_dir,
...
def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse Supplementary Figure 4 Spyglass renderer arguments."""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    path = render_supplementary_figure_4(
        payload,
        output_path=output_path,
        dpi=args.dpi,
    )
    print(f"Saved Spyglass Supplementary Figure 4 to {path}")
    if args.promote_to is not None:
        promoted = promote_supplementary_figure_4(
            payload,
            source_path=path,
            destination_path=args.promote_to,
            replace=args.replace_promoted_output,
        )
        print(f"Promoted Spyglass Supplementary Figure 4 to {promoted}")
...
__all__ = [
    ...
    "EXPECTED_DATASETS",
    "get_output_path",
    "load_supplementary_figure_4_payload",
    "main",
    "promote_supplementary_figure_4",
