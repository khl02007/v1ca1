"""Generate Supplementary Figure 3_2 from two existing summary panels."""
...
DEFAULT_OUTPUT_DIR = figure_3.DEFAULT_OUTPUT_DIR
DEFAULT_OUTPUT_NAME = "supplementary_figure_3_2"
DEFAULT_OUTPUT_FORMAT = figure_3.DEFAULT_OUTPUT_FORMAT
FIGURE_FORMATS = figure_3.FIGURE_FORMATS
DEFAULT_REGION = figure_3.DEFAULT_REGIONS[0]
...
def load_panel_a_decoding_data(
    *,
    ...
    """Load decoding data for Supplementary Figure 3_2 Panel A."""
    ...
    if (
        ...
        raise ValueError(
            "Supplementary Figure 3_2 decoding inference requires exactly one "
            "data set per animal; "
...
def make_supplementary_figure_3_2(
    *,
    data_root: Path,
    output_path: Path,
    datasets: Sequence[DatasetId],
    ...
    """Build and save Supplementary Figure 3_2 panels A and B."""
    ...
    plt.close(fig)
    print(f"Saved Supplementary Figure 3_2 to {output_path}")
    return output_path


def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse command-line arguments for Supplementary Figure 3_2."""
    parser = argparse.ArgumentParser(
        description="Generate Supplementary Figure 3_2 panels A and B."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> None:
    """Run Supplementary Figure 3_2 generation."""
    args = parse_arguments(argv)
    datasets = (
        args.dataset if args.dataset is not None else get_processed_datasets()
    )
    output_path = figure_3.build_output_path(
        args.output_dir,
        args.output_name,
        args.output_format,
    )
