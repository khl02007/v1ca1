"""Render Supplementary Figure 3 from retained Spyglass motor artifacts."""
...
from v1ca1.paper_figures import supplementary_figure_3 as canonical
...
DEFAULT_OUTPUT_NAME = "supplementary_figure_3_spyglass"
DEFAULT_OUTPUT_FORMAT = "svg"
DEFAULT_DPI = 300
EXPECTED_DATASETS = figure_2_adapter.EXPECTED_DATASETS
LIGHT_EPOCH = canonical.MOTOR_PANEL_LIGHT_EPOCH
FIGURE_ARTIFACT_KIND = "complete_spyglass_supplementary_figure_3"
    ...
    for session in sessions:
        ...
        for epoch_type, epoch in (("dark", dark_epoch), ("light", LIGHT_EPOCH)):
            ...
            if str(loaded.get("artifact_origin", "")) != "computed":
                raise ValueError(
                    "Supplementary Figure 3 requires computed motor artifacts."
                )
            ...
            tables.append(table)
    if not tables:
        raise ValueError("Supplementary Figure 3 has no motor artifacts.")
...
def load_supplementary_figure_3_payload(
    *,
    run_id: str,
    scratch_root: Path = DEFAULT_SCRATCH_ROOT,
) -> dict[str, Any]:
    """Load the active Supplementary Figure 3 inputs from one campaign."""
    ...
    def load_motor(
        *,
        ...
        if Path(data_root).resolve(strict=True) != Path(payload["run_dir"]).resolve(
            ...
            raise ValueError("Supplementary Figure 3 requested a foreign root.")
        observed = tuple(normalize_dataset_id(value) for value in datasets)
        if observed != tuple(payload["datasets"]):
            raise ValueError("Supplementary Figure 3 requested foreign sessions.")
        if str(light_epoch) != LIGHT_EPOCH:
            raise ValueError("Supplementary Figure 3 requested a foreign light epoch.")
...
def get_output_path(
    *,
    ...
    """Return the immutable run-local Supplementary Figure 3 path."""
...
def promote_supplementary_figure_3(
    ...
    *,
    ...
    """Publish one validated Supplementary Figure 3 and receipt."""
...
def render_supplementary_figure_3(
    payload: Mapping[str, Any],
    *,
    output_path: Path,
    dpi: int = DEFAULT_DPI,
) -> Path:
    """Atomically render Supplementary Figure 3 inside its campaign."""
    ...
    if output_path.exists() or provenance_path.exists():
        raise FileExistsError(
            f"Refusing to overwrite Supplementary Figure 3: {output_path}"
        )
    if output_path.suffix.lower().lstrip(".") not in canonical.FIGURE_FORMATS:
        raise ValueError("Unsupported Supplementary Figure 3 output format.")
    ...
    try:
        with _offline_sources(payload):
            rendered = canonical.make_supplementary_figure_3(
                data_root=run_dir,
...
def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse Supplementary Figure 3 Spyglass renderer arguments."""
...
def main(argv: Sequence[str] | None = None) -> None:
    """Load one supplementary campaign and render Supplementary Figure 3."""
    args = parse_arguments(argv)
    payload = load_supplementary_figure_3_payload(
        run_id=args.run_id,
    ...
    path = render_supplementary_figure_3(
        payload,
        output_path=output_path,
        dpi=args.dpi,
    )
    print(f"Saved Spyglass Supplementary Figure 3 to {path}")
    if args.promote_to is not None:
        promoted = promote_supplementary_figure_3(
            payload,
            source_path=path,
            destination_path=args.promote_to,
            replace=args.replace_promoted_output,
        )
        print(f"Promoted Spyglass Supplementary Figure 3 to {promoted}")
...
__all__ = [
    ...
    "EXPECTED_DATASETS",
    "get_output_path",
    "load_supplementary_figure_3_payload",
    "main",
    "promote_supplementary_figure_3",
