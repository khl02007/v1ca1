...
"""Generate Supplementary Figure 5 motor-control panels."""
...
PANEL_A_CACHE_PREFIX = "supplementary_figure_5_panel_a"
...
def load_panel_a_cv_pca_participation_ratio_table(
    *,
    ...
    """Load paired dark/light cvPCA participation ratios for Supplementary Figure 5A."""
    import pandas as pd
    ...
    if missing_paths:
        ...
        raise FileNotFoundError(
            "Missing Supplementary Figure 5A cvPCA summary files:\n" + message
        )
...
def load_panel_b_motor_progression_table(
    *,
    ...
    """Load motor progression summaries for the Supplementary Figure 5B grid."""
    import pandas as pd
    ...
    if missing_paths:
        message = "\n".join(f"- {path}" for path in missing_paths)
        raise FileNotFoundError(
            "Missing Supplementary Figure 5B motor progression files:\n" + message
        )
    if missing_entries:
        message = "\n".join(f"- {entry}" for entry in missing_entries)
        raise ValueError(
            "Motor progression summaries are incomplete for Supplementary Figure 5B:\n"
            + message
        )
    if not tables:
        raise ValueError("No motor progression tables were loaded for Supplementary Figure 5B.")
...
def plot_panel_b_motor_progression_grid(
    ...
    *,
    ...
    if not dataset_order:
        raise ValueError(
            "Supplementary Figure 5B is configured to plot "
    ...
    if panel_table.empty:
        raise ValueError(
            "Supplementary Figure 5B motor table has no rows for "
...
def _format_panel_a_cache_token(value: object) -> str:
    """Return a filesystem-safe token for Supplementary Figure 5A caches."""
...
def _format_panel_a_cache_number(value: float | int) -> str:
    """Return a compact numeric token for Supplementary Figure 5A caches."""
...
def _build_panel_a_dataset_cache_token(
    ...
    """Return a descriptive cache token for the Supplementary Figure 5A data sets."""
    dataset_tokens = [
...
def build_panel_a_cache_metadata(
    *,
    ...
    """Return metadata identifying one Supplementary Figure 5A heatmap cache."""
...
def build_panel_a_cache_path(cache_dir: Path, metadata: dict[str, Any]) -> Path:
    """Return the cache path for one Supplementary Figure 5A heatmap payload."""
...
def _panel_a_cache_array_name(order_trajectory: str, plot_trajectory: str) -> str:
    """Return the cache array name for one Supplementary Figure 5A panel."""
    return f"{order_trajectory}__{plot_trajectory}"


def _panel_a_cache_unit_order_array_name(order_trajectory: str) -> str:
    """Return the cache array name for one Supplementary Figure 5A unit order."""
...
def save_panel_a_cache(
    ...
    """Write one Supplementary Figure 5A heatmap cache."""
...
def load_panel_a_cache(
    ...
    """Return cached Supplementary Figure 5A heatmaps when metadata matches."""
    cache_path = Path(cache_path)
    ...
    try:
        with np.load(cache_path, allow_pickle=False) as data:
            ...
            if cached_metadata != expected_metadata:
                print(f"Ignoring stale Supplementary Figure 5A cache at {cache_path}.")
                return None
        ...
        print(f"Ignoring unreadable Supplementary Figure 5A cache at {cache_path}: {exc}")
        return None
...
def build_panel_a_heatmap_payloads(
    *,
    ...
    """Return the Supplementary Figure 5A Fig. 1D-order heatmap payload."""
    ...
    *,
    ...
    if not refresh_panel_a_cache:
        for order_mode, cache_path in cache_paths_by_order_mode.items():
            ...
            if cached_payload is not None:
                print(f"Loaded Supplementary Figure 5A cache from {cache_path}.")
    ...
    if missing_order_modes:
        ...
        for order_mode in missing_order_modes:
            ...
            print(f"Saved Supplementary Figure 5A cache to {cache_path}.")
...
def add_supplementary_figure_5_bc_panels(
    fig: Any,
    motor_grid_spec: Any,
    ...
    *,
    ...
    """Add the shared Supplementary Figure 5 panels B and C."""
...
def make_supplementary_figure_5(
    *,
    ...
    """Build and save Supplementary Figure 5 panels A and B."""
    ...
    motor_axes, motor_summary_axes = add_supplementary_figure_5_bc_panels(
        fig,
        outer_grid[0, 0],
        outer_grid[1, 0],
        outer_grid[2, 0],
...
def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse command-line arguments for Supplementary Figure 5 generation."""
    parser = argparse.ArgumentParser(
        description="Generate Supplementary Figure 5 motor-control panels."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
