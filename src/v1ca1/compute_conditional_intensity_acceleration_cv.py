...
def load_classifiers(
    ...
    acc_bin_size: float = 1.0,
    movement_var: float = 4.0,
):
    print(f"loading classifiers")
    epoch = run_epoch_list[epoch_idx]

    classifier_save_dir = analysis_path / "classifier_acceleration_cv"
    ...
    for region in regions:
        ...
        for fold_idx in range(n_folds):
            classifier_save_path = classifier_save_dir / (
                ...
                f"_acceleration_{fold_idx}_of_{n_folds}_folds"
                f"_movement_{movement}"
                f"_fit_acc_std_{acc_std}"
                f"_discrete_var_{discrete_var}"
                f"_acc_bin_size_{acc_bin_size}"
...
def compute_rate_function_acceleration_cv(
    epoch_idx: int,
    n_folds: int,
    movement: str = "True",
    acc_std: float = 4.0,
    ...
    rate_function_save_dir = analysis_path / "rate_function_acceleration_cv"
    rate_function_save_dir.mkdir(parents=True, exist_ok=True)

    log_likelihood_save_dir = analysis_path / "log_likelihood_acceleration_cv"
    ...
    print(
        "Computing rate function by trajectory from acceleration fields "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
        f"n_folds {n_folds}, "
        f"movement {movement}, "
    ...
    for region in regions:
        ...
        f_acc = scipy.interpolate.interp1d(
            t_position, acceleration, axis=0, bounds_error=False, kind="linear"
        )
        acc_interp = f_acc(time)
        ...
        for unit_id in sorting[region].get_unit_ids():
            ...
            for trajectory_type in trajectory_types:
                ...
                for fold_idx in range(n_folds):
                    ...
                    acc_bin_centers = classifier.place_fields_[("", 0)][
                        "position"
                    ].to_numpy()
                    acc_field = classifier.place_fields_[("", 0)][:, unit_id].to_numpy()
                    ...
                    for traj_start_time, traj_stop_time in trajectory_times[epoch][
                        ...
                        r = f_rate(acc_interp[inds_to_keep])
                        nans = np.isnan(r)
                        if np.any(nans):
                            good = ~nans
                            if good.sum() == 0:
...
def main():
    ...
    compute_rate_function_acceleration_cv(
        epoch_idx=args.epoch_idx,
        n_folds=args.n_folds,
        movement=args.movement,
        acc_std=args.acc_std,
        discrete_var=args.discrete_var,
        acc_bin_size=args.acc_bin_size,
