...
# fit
def fit_headdir_speed_decode(
    epoch_idx,
    use_half="all",
    movement="True",
    headdir_speed_std=10,
    discrete_var="switching",
    headdir_speed_bin_size=5,
    movement_var=25,
    overwrite="False",
):
    if movement == "False":
    ...
    print(
        f"Fitting head direction speed decoder "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
    ...
    for region in regions:
        ...
        environment = rtc.Environment(
            place_bin_size=headdir_speed_bin_size,
        )
        ...
        classifier = rtc.SortedSpikesClassifier(
            ...
            sorted_spikes_algorithm_params={
                "position_std": headdir_speed_std,
                "use_diffusion": False,
                "block_size": int(2**11),
            },
        )
        ...
        # fit classifiers
        classifier.fit(
            head_direction_speed_interp[inds_to_keep],
