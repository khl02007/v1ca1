...
# fit
def fit_acceleration_decode_cv(
    epoch_idx,
    n_folds=10,
    movement="True",
    acc_std=2.0,
    ...
    print(
        f"Fitting acceleration decoder "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
        f"n_folds {n_folds}, "
        f"movement {movement}, "
...
def parse_arguments():
    ...
    parser.add_argument(
        "--acc_std",
        type=float,
        help="Width of gaussian for estimating acc fields; float",
    )
    parser.add_argument(
        "--discrete_var",
        type=str,
