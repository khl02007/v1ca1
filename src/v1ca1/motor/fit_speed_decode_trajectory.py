...
# fit
def fit_speed_decode_trajectory(
    epoch_idx,
    use_half="all",
    movement="True",
    speed_std=2.0,
    ...
    print(
        f"Fitting speed decoder by trajectory type "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
        ...
        for trajectory_type in trajectory_types:
            ...
            for traj_start_time, traj_stop_time in trajectory_times[epoch][
                ...
                if inds_to_keep.sum() == 0:
                    continue
                traj_speed.append(speed_interp[inds_to_keep])
                spike_indicator_traj.append(spike_indicator[region][inds_to_keep])

            traj_speed = np.concatenate(traj_speed, axis=0)
