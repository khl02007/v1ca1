...
"""Legacy-style ripple GLM outputs backed by modern session artifacts.
...
Unlike the legacy script, it loads canonical modern inputs from this repo:
`timestamps_ephys.npz`, `timestamps_ephys_all.npz`, `ripple/ripple_times.parquet`,
and `sorting_v1` / `sorting_ca1`.
"""
...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fit the legacy ripple GLM workflow from modern repo artifacts."
    )
...
def validate_selected_epochs_across_sources(
    ...
    *,
    ...
    if missing_by_source:
        ...
        raise ValueError(
            "Selected epochs are missing required ripple_glm2 inputs. "
...
def load_modern_ripple_tables(analysis_path: Path) -> dict[str, pd.DataFrame]:
    """Load ripple events from the canonical flattened parquet table."""
    parquet_path = require_existing_path(
        analysis_path / "ripple" / "ripple_times.parquet",
        description="ripple parquet output",
    )
    return load_ripple_tables_from_parquet_output(parquet_path)


def compare_ripple_event_sources(
    modern_tables: dict[str, pd.DataFrame],
    legacy_tables: dict[str, pd.DataFrame],
    *,
    atol_s: float = 1e-4,
) -> list[dict[str, Any]]:
    """Return per-epoch ripple mismatches between modern and legacy sources."""
    mismatches: list[dict[str, Any]] = []
    for epoch in sorted(set(modern_tables) | set(legacy_tables)):
        modern_table = modern_tables.get(epoch)
        legacy_table = legacy_tables.get(epoch)
        if modern_table is None or legacy_table is None:
            mismatches.append(
                {
                    "epoch": epoch,
                    "reason": "epoch_missing",
                    "modern_count": 0 if modern_table is None else int(len(modern_table)),
                    "legacy_count": 0 if legacy_table is None else int(len(legacy_table)),
                }
            )
            continue

        modern_start = modern_table["start_time"].to_numpy(dtype=float)
        modern_end = modern_table["end_time"].to_numpy(dtype=float)
        legacy_start = legacy_table["start_time"].to_numpy(dtype=float)
        legacy_end = legacy_table["end_time"].to_numpy(dtype=float)

        count_match = modern_start.shape == legacy_start.shape
        times_match = (
            count_match
            and np.allclose(modern_start, legacy_start, rtol=0.0, atol=atol_s)
            and np.allclose(modern_end, legacy_end, rtol=0.0, atol=atol_s)
        )
        if count_match and times_match:
            continue

        mismatches.append(
            {
                "epoch": epoch,
                "reason": "count_or_time_mismatch",
                "modern_count": int(len(modern_table)),
                "legacy_count": int(len(legacy_table)),
                "max_abs_start_diff_s": (
                    None
                    if not count_match
                    else float(np.max(np.abs(modern_start - legacy_start)))
                ),
                "max_abs_end_diff_s": (
                    None
                    if not count_match
                    else float(np.max(np.abs(modern_end - legacy_end)))
                ),
            }
        )
    return mismatches
...
def warn_if_legacy_ripple_pickle_differs(
    analysis_path: Path,
    modern_tables: dict[str, pd.DataFrame],
) -> None:
    """Warn when the legacy ripple pickle disagrees with the modern parquet table."""
    ...
    mismatches = compare_ripple_event_sources(modern_tables, legacy_tables)
    if not mismatches:
        return

    preview = "; ".join(
        (
            f"{item['epoch']}: modern={item['modern_count']} "
            f"legacy={item['legacy_count']}"
        )
        for item in mismatches[:5]
    )
    warnings.warn(
        "ripple_glm2 is using ripple/ripple_times.parquet, but it does not match "
        "the legacy ripple/Kay_ripple_detector.pkl for this session. Legacy-vs-modern "
        f"comparisons are not valid until the ripple event sources are aligned. "
        f"First mismatches: {preview}",
        stacklevel=2,
    )


def load_modern_epoch_timestamps(
    analysis_path: Path,
) -> tuple[list[str], dict[str, np.ndarray], dict[str, "nap.IntervalSet"]]:
    """Load canonical epoch timestamps and interval bounds from `.npz` exports."""
    require_existing_path(analysis_path / "timestamps_ephys.npz", description="timestamps_ephys.npz")
    require_existing_path(
        analysis_path / "timestamps_ephys_all.npz",
        description="timestamps_ephys_all.npz",
    )

    epoch_tags, timestamps_by_epoch, source = load_ephys_timestamps_by_epoch(analysis_path)
    if source != "pynapple":
        raise ValueError(
            "ripple_glm2 requires timestamps_ephys.npz and will not fall back to pickle."
        )
    return epoch_tags, timestamps_by_epoch, build_epoch_intervals(timestamps_by_epoch)


def load_modern_ephys_timestamps_all(analysis_path: Path) -> np.ndarray:
    """Load canonical concatenated ephys timestamps from `.npz`."""
    require_existing_path(
        analysis_path / "timestamps_ephys_all.npz",
        description="timestamps_ephys_all.npz",
    )
    timestamps_ephys_all, source = load_ephys_timestamps_all(analysis_path)
    if source != "pynapple":
        raise ValueError(
            "ripple_glm2 requires timestamps_ephys_all.npz and will not fall back to pickle."
        )
    return timestamps_ephys_all


def load_modern_spikes(
    analysis_path: Path,
    timestamps_ephys_all: np.ndarray,
) -> dict[str, "nap.TsGroup"]:
    """Load `sorting_v1` and `sorting_ca1` into pynapple TsGroups."""
...
def prepare_ripple_glm2_session(
    animal_name: str,
    date: str,
    data_root: Path = DEFAULT_DATA_ROOT,
) -> dict[str, Any]:
    """Load one session using the canonical modern ripple GLM inputs only."""
    ...
    epoch_tags, timestamps_ephys_by_epoch, epoch_intervals = load_modern_epoch_timestamps(
        analysis_path
    )
    timestamps_ephys_all = load_modern_ephys_timestamps_all(analysis_path)
    spikes = load_modern_spikes(analysis_path, timestamps_ephys_all)
    loaded_ripple_tables = load_modern_ripple_tables(analysis_path)
    warn_if_legacy_ripple_pickle_differs(analysis_path, loaded_ripple_tables)
...
def run_train_on_ripple(
    *,
    ...
    out_dir = session["analysis_path"] / "ripple" / "glm_results_train_on_ripple"
    fig_dir = session["analysis_path"] / "figs" / "ripple" / "train_on_ripple"
...
def run_ridge_sweep(
    *,
    ...
    """Run the legacy ridge-sweep utility on modern inputs."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not selected_epochs:
        raise ValueError(
            "No selected epochs are present in ripple/ripple_times.parquet. "
    ...
    log_path = write_run_log(
        ...
        script_name="v1ca1.ripple.ripple_glm2",
        parameters={
            "animal_name": args.animal_name,
            "date": args.date,
            "data_root": args.data_root,
