...
def load_classifiers(
    ...
    classifier_save_dir = analysis_path / "classifier_speed_cv"
    ...
    for region in regions:
        ...
        for fold_idx in range(n_folds):
            classifier_save_path = classifier_save_dir / (
                ...
                f"_speed_{fold_idx}_of_{n_folds}_folds"
...
def compute_rate_function_speed_cv(
    epoch_idx: int,
    n_folds: int,
    movement: str = "True",
    speed_std: float = 4.0,
    ...
    rate_function_save_dir = analysis_path / "rate_function_speed_cv"
    rate_function_save_dir.mkdir(parents=True, exist_ok=True)

    log_likelihood_save_dir = analysis_path / "log_likelihood_speed_cv"
        ...
        for unit_id in sorting[region].get_unit_ids():
            ...
            for trajectory_type in trajectory_types:
                ...
                for fold_idx in range(n_folds):
                    ...
                    speed_bin_centers = classifier.place_fields_[("", 0)][
                        "position"
                    ].to_numpy()
...
def main():
    ...
    compute_rate_function_speed_cv(
