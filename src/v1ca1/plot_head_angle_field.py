...
def plot_head_angle_field(
    headdir_std=1.0,
):

    fig_save_path = analysis_path / "figs" / "head_angle_field"
    ...
    for region in regions:
        ...
        for unit_id in sorting[region].get_unit_ids():
                ...
                for epoch_idx, epoch in enumerate(run_epoch_list):
                    ...
                    for traj_start_time, traj_end_time in trajectory_times[epoch][
                        ...
                        if len(spike_times_trajectory) == 0:
                            ...
                            continue
                        unwrapped_head_direction_given_spikes = (
                            f_head_direction_unwrapped(spike_times_trajectory)
                        )
                        wrapped_head_direction_given_spikes = (
                            unwrapped_head_direction_given_spikes + 180
                        ) % 360 - 180

                        position_list.append(wrapped_head_direction_given_spikes)
                    ...
                    ax[ax_row_offset + epoch_idx, ax_col_offset].set_xlim([-180, 180])
            ...
            ax[-1, -1].set_xlabel("Head angle (deg)")
