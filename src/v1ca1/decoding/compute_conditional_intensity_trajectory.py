...
def compute_rate_function_trajectory(
    epoch_idx: int,
    use_half: str,
    movement: str = "True",
    ...
    rate_function_save_dir = analysis_path / "rate_function_trajectory"
    rate_function_save_dir.mkdir(parents=True, exist_ok=True)

    log_likelihood_save_dir = analysis_path / "log_likelihood_trajectory"
    log_likelihood_save_dir.mkdir(parents=True, exist_ok=True)

    spike_indicators_save_dir = analysis_path / "spike_indicators_trajectory_trial"
    ...
    print(
        f"Computing rate function by trajectory "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
        f"use_half {use_half}, "
        f"movement {movement}, "
    ...
    for region in regions:
        ...
        for trajectory_type in trajectory_types:
            ...
            spike_indicators[region][trajectory_type] = {}
            ...
            position_bin_centers = classifier.place_fields_[("", 0)][
                "position"
            ].to_numpy()
            ...
            for unit_index in unit_indices:
                place_field = classifier.place_fields_[("", 0)][
                    :, unit_index
                ].to_numpy()
                ...
                for traj_start_time, traj_stop_time in trajectory_times[epoch][
                    ...
                    position_df = tl.get_linearized_position(
                        position=head_position_2d_interp[inds_to_keep],
                        track_graph=track_graph,
                        edge_order=linear_edge_order,
                        edge_spacing=linear_edge_spacing,
                    )
                    head_position_1d_trajectory = position_df[
                        "linear_position"
                    ].to_numpy()
                    r = f_rate(head_position_1d_trajectory)
                    r = interpolate_nonmonotonic_x(head_position_1d_trajectory, r)
                    ...
                    spike_indicator_list.append(bins_with_spikes)
...
def main():
    ...
    compute_rate_function_trajectory(
        args.epoch_idx,
        args.use_half,
        args.movement,
        args.position_std,
