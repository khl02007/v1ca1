...
classifier_save_dir = analysis_path / "classifier_headdir_speed"
...
def compute_rate_function_headdir_speed(
    epoch_idx: int,
    use_half: str,
    movement: str = "True",
    ...
    rate_function_save_dir = analysis_path / "rate_function_headdir_speed"
    rate_function_save_dir.mkdir(parents=True, exist_ok=True)

    log_likelihood_save_dir = analysis_path / "log_likelihood_headdir_speed"
    ...
    for region in regions:
        ...
        head_direction_speed = np.abs(head_direction_deg_velocity)
        f_head_direction_speed = scipy.interpolate.interp1d(
            t_position, head_direction_speed, axis=0, bounds_error=False, kind="linear"
        )
        head_direction_speed_interp = f_head_direction_speed(time)
        ...
        for trajectory_type in trajectory_types:
            ...
            classifier_save_path = classifier_save_dir / (
                f"classifier_{region}_{date}_{epoch}_headdir_speed"
                f"_use_half_{use_half}"
                f"_movement_{movement}"
            ...
            for unit_index in unit_indices:
                ...
                for traj_start_time, traj_stop_time in trajectory_times[epoch][
                    ...
                    r = interpolate_nonmonotonic_x(
                        head_direction_speed_interp[inds_to_keep], r
                    )
...
def main():
    ...
    compute_rate_function_headdir_speed(
