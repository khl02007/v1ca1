    ...
    for region in regions:
        ...
        for fold_idx in range(n_folds):
            classifier_save_path = classifier_save_dir / (
                ...
                f"_headdir_velocity_{fold_idx}_of_{n_folds}_folds"
...
def compute_rate_function_headdir_velocity_cv(
    epoch_idx: int,
    n_folds: int,
    movement: str = "True",
    headdir_velocity_std: float = 4.0,
    ...
    rate_function_save_dir = analysis_path / "rate_function_headdir_velocity_cv"
    rate_function_save_dir.mkdir(parents=True, exist_ok=True)

    log_likelihood_save_dir = analysis_path / "log_likelihood_headdir_velocity_cv"
    ...
    classifier_dict = load_classifiers(
        ...
        headdir_velocity_bin_size=headdir_velocity_bin_size,
...
def main():
    ...
    compute_rate_function_headdir_velocity_cv(
