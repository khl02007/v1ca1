...
def compute_rate_function_speed(
    epoch_idx: int,
    use_half: str,
    movement: str = "True",
    speed_std: float = 4.0,
    discrete_var: str = "switching",
    speed_bin_size: float = 1.0,
    ...
    rate_function_save_dir = analysis_path / "rate_function_speed"
    rate_function_save_dir.mkdir(parents=True, exist_ok=True)

    log_likelihood_save_dir = analysis_path / "log_likelihood_speed"
    ...
    for region in regions:
        ...
        for trajectory_type in trajectory_types:
            ...
            classifier_save_path = classifier_save_dir / (
                f"classifier_{region}_{date}_{epoch}_speed"
                f"_use_half_{use_half}"
                f"_movement_{movement}"
            ...
            speed_bin_centers = classifier.place_fields_[("", 0)]["position"].to_numpy()

            for unit_index in unit_indices:
                speed_field = classifier.place_fields_[("", 0)][
                    :, unit_index
                ].to_numpy()
                ...
                for traj_start_time, traj_stop_time in trajectory_times[epoch][
                    ...
                    r = f_rate(speed_interp[inds_to_keep])
                    r = interpolate_nonmonotonic_x(speed_interp[inds_to_keep], r)
