...
def load_classifier_dict(
    ...
    headdir_bin_size=2.0,
    movement_var=25.0,
):
    classifier_save_dir = analysis_path / "classifier_headdir_trajectory"
...
def plot_headdir_field_trajectory(
    headdir_std=5.0,
):

    fig_save_path = analysis_path / "figs" / "headdir_field_trajectory"
    fig_save_path.mkdir(parents=True, exist_ok=True)

    classifier_dict = load_classifier_dict(headdir_std=headdir_std)
        ...
        for unit_id in sorting[region].get_unit_ids():
            ...
            for trajectory_type in trajectory_types:
                ...
                for epoch_idx, epoch in enumerate(run_epoch_list):
                    if not trajectory_type in classifier_dict[region][epoch]:
                        continue
                    ...
                    head_direction_interp = head_direction_interp * 180 / np.pi
                    ...
                    for traj_start_time, traj_end_time in trajectory_times[epoch][
                        ...
                        headdir_list.append(f_headdir(spike_times_trajectory))

                    for i, pos_seg in enumerate(headdir_list):
                        if len(pos_seg) == 0:
                    ...
                    num_trials = sum(len(seg) > 0 for seg in headdir_list)
                    ...
                    ax[ax_row_offset + epoch_idx, ax_col_offset].set_xlim([0, 360])
            ...
            ax[-1, -1].set_xlabel("Head direction (deg)")
            ax[-1, -1].set_ylabel("Trials")

            fig.suptitle(f"{animal_name} {date} {region} {unit_id}")

            fig.savefig(
                fig_save_path / f"{region}_{unit_id}_{date}.png",
                dpi=300,
                bbox_inches="tight",
            )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--headdir_std",
        type=float,
        help="std of gaussian kernel for estimating head direction fields, in degrees",
    )
