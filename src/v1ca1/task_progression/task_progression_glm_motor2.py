...
Overview
--------
We compare three Poisson GLMs (exp link) using cross-validated log-likelihood
...
Where
  ...
  • X_motor(t) are motor covariates shared across trajectories. By default these
    are used **instantaneously** with **z-scoring** before model fit; you can
    switch back to spline-expanded motor covariates with
    `motor_feature_mode="spline"`.
  • X_traj(t) is a trajectory-group dummy so models can have different mean rates
    across the (user-defined) trajectory groups in `TP_GROUPS`.
  • tp(t) is task progression: linearized fraction along the current trajectory,
    always in ~[0,1].
  • I_j(t) is a 0/1 indicator for the *current* trajectory group. This “gates”
    task progression so each group gets its own TP tuning curve.
...
The main function `fit_motor_vs_task_progression_wtrack_epoch` returns:
  • CV bits/spike for all models (overall and per trajectory)
...
def fit_motor_vs_task_progression_wtrack_epoch(
    *,
    ...
    2) Computes *motor covariates* (speed, accel, head direction components,
       angular velocity) and either:
         • uses them **instantaneously** with **z-scoring** (default), or
         • spline-expands them (legacy behavior; set motor_feature_mode="spline").

    3) Computes per-trajectory task progression (linearized distance fraction
       along that trajectory). Task progression lives in ~[0,1] and is clipped
       into `tp_bounds` for robustness.

    4) Fits three models with cross-validation:
       - M0: motor-only (plus trajectory-group dummy)
       - M1: motor + trajectory-gated task-progression spline
       - M2: task progression only (no motor), plus trajectory-group dummy

       Importantly:
         • Motor coefficients are shared across trajectories.
         • Task-progression coefficients are *separate for each trajectory group*
           defined in `TP_GROUPS` via 0/1 gating.
    ...
    tp_spline_k, tp_spline_order, tp_bounds
        Task-progression spline basis settings. Default bounds (0,1) match the
        fact that TP is a per-trajectory fraction.
    motor_spline_k, motor_spline_order
        Spline basis settings for motor covariates (only used when
        motor_feature_mode="spline").
    motor_feature_mode
        "zscore" (default) uses instantaneous motor variables with z-scoring
        prior to model fit.
        "spline" keeps the previous spline-expanded motor covariates.
    motor_zscore_eps
        Small constant used to avoid divide-by-zero when z-scoring.
    ridge
    ...
    results
        A dict with keys:
          ...
          • 'coef' : coefficients from refits on all data for all models
    ...
    # Task progression here is a fraction along the current trajectory, so it
    # should live in ~[0,1]. We still clip into `tp_bounds` to be robust to small
    # linearization/interpolation overshoots.
    tp_lo, tp_hi = map(float, tp_bounds)
    if not np.isfinite(tp_lo) or not np.isfinite(tp_hi) or tp_hi <= tp_lo:
        raise ValueError(f"Invalid tp_bounds={tp_bounds}; expected finite (lo < hi).")
    ...
    # ---- Motor design matrix ----
    # By default we use *instantaneous* motor variables with z-scoring (like the
    # "speed" handling in your other script), but we keep the spline-expanded
    # version as an option for backwards compatibility.
    motor_names: List[str] = []
    motor_standardization: Dict[str, object] | None = None
    ...
    if motor_feature_mode == "spline":
        ...
        # Continuous motor variables (one column each) -> z-score.
        motor_raw_names = ["speed", "accel", "hd_vel", "abs_hd_vel", "sin_hd", "cos_hd"]
        X_motor_raw = np.column_stack([cov_k[n] for n in motor_raw_names]).astype(float)

        mu = X_motor_raw.mean(axis=0)
        sd = X_motor_raw.std(axis=0)

        sd_safe = sd.copy()
        bad = (~np.isfinite(sd_safe)) | (sd_safe < float(motor_zscore_eps))
        sd_safe[bad] = 1.0

        X_motor = (X_motor_raw - mu) / sd_safe
        # If a feature is effectively constant, set it to 0 so it can't cause
        # numerical issues (and it contains no predictive information anyway).
        if np.any(bad):
            X_motor[:, bad] = 0.0

        motor_names = [f"{n}_z" for n in motor_raw_names]
        P_motor = X_motor.shape[1]
    ...
    # M2: TP-only (no motor)
    X2 = np.concatenate([X_traj, X_tp], axis=1)
    ...
    ll2_sum = np.zeros(U, dtype=float)
    ...
    ll2_sum_by_traj = {traj: np.zeros(U, dtype=float) for traj in TRAJECTORY_TYPES}
    ...
    for tr, te in folds:
        ...
        m2 = PopulationGLM("Poisson", regularizer="Ridge", regularizer_strength=ridge)
        m0.fit(X0[tr], y[tr])
        m1.fit(X1[tr], y[tr])
        m2.fit(X2[tr], y[tr])
    ...
    ll2_ps = _per_spike(ll2_sum, spk_sum)

    cv_pooled = {
        "spike_sum": spk_sum,
        "ll_motor_bits_per_spike": ll0_ps * inv_log2,
        "ll_motor_tp_bits_per_spike": ll1_ps * inv_log2,
        "ll_tp_only_bits_per_spike": ll2_ps * inv_log2,
        "dll_bits_per_spike": (ll1_ps - ll0_ps) * inv_log2,
        "dll_tp_only_vs_motor_bits_per_spike": (ll2_ps - ll0_ps) * inv_log2,
        "dll_motor_tp_vs_tp_only_bits_per_spike": (ll1_ps - ll2_ps) * inv_log2,
    }
    ...
    for traj in TRAJECTORY_TYPES:
        ...
        ll2_t = ll2_sum_by_traj[traj]
        spk_t = spk_sum_by_traj[traj]
        ll0_ps_t = _per_spike(ll0_t, spk_t)
        ll1_ps_t = _per_spike(ll1_t, spk_t)
        ll2_ps_t = _per_spike(ll2_t, spk_t)
        cv_by_traj[traj] = {
            "spike_sum": spk_t,
            "ll_motor_bits_per_spike": ll0_ps_t * inv_log2,
            "ll_motor_tp_bits_per_spike": ll1_ps_t * inv_log2,
            "ll_tp_only_bits_per_spike": ll2_ps_t * inv_log2,
            "dll_bits_per_spike": (ll1_ps_t - ll0_ps_t) * inv_log2,
            "dll_tp_only_vs_motor_bits_per_spike": (ll2_ps_t - ll0_ps_t) * inv_log2,
            "dll_motor_tp_vs_tp_only_bits_per_spike": (ll1_ps_t - ll2_ps_t) * inv_log2,
        }
    ...
    m2_all = PopulationGLM("Poisson", regularizer="Ridge", regularizer_strength=ridge)
    m0_all.fit(X0, y)
    m1_all.fit(X1, y)
    m2_all.fit(X2, y)
    ...
    coef2 = _coef_feat_by_unit(m2_all, n_features=X2.shape[1])
    intercept0 = np.asarray(m0_all.intercept_).reshape(-1)
    intercept1 = np.asarray(m1_all.intercept_).reshape(-1)
    intercept2 = np.asarray(m2_all.intercept_).reshape(-1)
    ...
    # ---- TP-only coefficients ----
    # X2 = [X_traj, X_tp]
    coef2_traj = coef2[:P_traj, :]
    coef2_tp = coef2[P_traj:, :]  # (P_tp, U)

    coef2_tp_by_group: Dict[str, np.ndarray] = {}
    for gi, (gname, _) in enumerate(TP_GROUPS):
        coef2_tp_by_group[gname] = coef2_tp[gi * Ktp : (gi + 1) * Ktp, :]

    coef2_tp_by_traj: Dict[str, np.ndarray] = {}
    for gi, (gname, (ta, tb)) in enumerate(TP_GROUPS):
        coef2_tp_by_traj[ta] = coef2_tp_by_group[gname]
        coef2_tp_by_traj[tb] = coef2_tp_by_group[gname]
    ...
    motor_mean = (
        np.zeros(P_motor, dtype=float)
        if motor_feature_mode == "zscore"
        else X_motor.mean(axis=0)
    )
    ...
    for k, traj in enumerate(TRAJECTORY_TYPES):
        grid = np.linspace(tp_lo, tp_hi, 200)
    ...
    return {
        ...
        "motor_feature_mode": str(motor_feature_mode),
        ...
        "coef": {
            ...
            "tp_only": {
                ...
                "coef_traj": coef2_traj,
                "coef_tp_by_group": coef2_tp_by_group,
                "coef_tp_by_traj": coef2_tp_by_traj,
            },
        },
