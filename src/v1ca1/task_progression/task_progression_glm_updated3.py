...
from typing import Dict, Hashable, List, Sequence, Tuple, Any
...
def _zscore_train_apply(
    ...
    mu = float(np.mean(v[train_idx]))
    sd = float(np.std(v[train_idx]) + eps)
    v_train = (v[train_idx] - mu) / sd
    v_test = (v[test_idx] - mu) / sd
    return v_train, v_test, mu, sd
...
def fit_shared_place_light_mod_nemos_per_traj(
    ...
    *,
    ...
    # Design matrices
    x_base_nospeed = np.concatenate([B, l_all[:, None]], axis=1)  # (T, K+1)
    ...
    for tr, te in folds:
        if has_speed:
            v_tr, v_te, _, _ = _zscore_train_apply(v_all, tr, te)
            x_base_tr = np.concatenate([x_base_nospeed[tr], v_tr[:, None]], axis=1)
            x_base_te = np.concatenate([x_base_nospeed[te], v_te[:, None]], axis=1)
            x_full_tr = np.concatenate([x_full_nospeed[tr], v_tr[:, None]], axis=1)
            x_full_te = np.concatenate([x_full_nospeed[te], v_te[:, None]], axis=1)
        else:
            x_base_tr = x_base_nospeed[tr]
            x_base_te = x_base_nospeed[te]
            x_full_tr = x_full_nospeed[tr]
            x_full_te = x_full_nospeed[te]
        ...
        m0.fit(x_base_tr, y_all[tr])
        m1.fit(x_full_tr, y_all[tr])
    ...
    m0_all.fit(x_base_all, y_all)
    m1_all.fit(x_full_all, y_all)

    coef0 = _coef_feat_by_unit(m0_all, n_features=x_base_all.shape[1])  # (P0,U)
    coef1 = _coef_feat_by_unit(m1_all, n_features=x_full_all.shape[1])  # (P1,U)
...
def fit_shared_place_light_mod_nemos_per_traj_segment_gain(
    ...
    *,
    ...
    # Design matrices
    x_base_nospeed = np.concatenate([B_dark, l_all[:, None]], axis=1)  # (T, Kd+1)
    x_full_nospeed = np.concatenate(
    ...
    coef0 = _coef_feat_by_unit(m0_all, n_features=x_base_all.shape[1])
    coef1 = _coef_feat_by_unit(m1_all, n_features=x_full_all.shape[1])
...
def fit_shared_place_light_mod_nemos_per_traj_segment_scalar_gain(
    ...
    *,
    ...
    # --- Design matrices ---
    x_base_nospeed = np.concatenate([B_dark, l_all[:, None]], axis=1)
    x_full_nospeed = np.concatenate(
        [B_dark, l_all[:, None], B_seg * l_all[:, None]], axis=1
    )
...
def fit_true_additive_rate_poisson_fast_per_traj(
    ...
    *,
    ...
    for tr, te in folds:
        ...
        if has_speed:
            v_tr, v_te, _, _ = _zscore_train_apply(v_all, tr, te, eps=1e-6)
    ...
    if has_speed:
        ...
        v_all_z = (v_all - speed_mean) / speed_std
        x_base_all = np.concatenate([B_all, v_all_z[:, None]], axis=1)
    ...
    pars_all = _fit_additive_rate_batched_jax(
        ...
        v=None if not has_speed else np.asarray(v_all_z, dtype=np.float32),
...
def fit_separate_dark_light_fields_nemos_per_traj(
    ...
    *,
    ...
    x_full_nospeed = np.concatenate([Bd, Bl, l_all[:, None]], axis=1)  # (T, 2K+1)
...
def main():
    ...
    save_dir = analysis_path / "tp_glm_updated3"
