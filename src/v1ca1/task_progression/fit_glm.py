...
def fit_glm(region, epoch, n_folds=5):
    ...
    for unit_id in list(spike_counts.columns):
        scores[unit_id] = {}
        predicted_rates[unit_id] = {}
        for model in models:
            scores[unit_id][model] = {}
            predicted_rates[unit_id][model] = {}
    ...
    with open(glm_results_save_path / f"{epoch}_{region}_scores.pkl", "wb") as f:
        pickle.dump(scores, f)
    with open(glm_results_save_path / f"{epoch}_{region}_rates.pkl", "wb") as f:
        pickle.dump(predicted_rates, f)
    with open(glm_tc_save_path / f"{epoch}_{region}_glm_place_fields.pkl", "wb") as f:
        pickle.dump(glm_place_fields, f)
...
def main():
    ...
    for region in regions:
        for epoch in run_epoch_list:
            fit_glm(region, epoch, n_folds=5)
