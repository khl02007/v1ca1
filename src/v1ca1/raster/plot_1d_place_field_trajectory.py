...
def plot_1d_place_field_trajectory(
    position_std=4.0,
):

    fig_save_path = analysis_path / "figs" / "place_field_1d_trajectory"
        ...
        for unit_id in sorting[region].get_unit_ids():
            ...
            for trajectory_type in trajectory_types:
                ...
                for epoch_idx, epoch in enumerate(run_epoch_list):
                    ...
                    f_position_2d = scipy.interpolate.interp1d(
                        timestamps_position,
                        position,
                        axis=0,
                        bounds_error=False,
                    ...
                    for traj_start_time, traj_end_time in trajectory_times[epoch][
                        ...
                        position_df = tl.get_linearized_position(
                            position=f_position_2d(spike_times_trajectory),
                            track_graph=track_graph,
                            edge_order=linear_edge_order,
                            edge_spacing=linear_edge_spacing,
                        )
                        position_list.append(position_df["linear_position"].to_numpy())
                    ...
                    ax[ax_row_offset + epoch_idx, ax_col_offset].set_xlim(
                        [
                            0,
                            long_segment_length * 2
                            + short_segment_length
...
def main():
    ...
    plot_1d_place_field_trajectory(
