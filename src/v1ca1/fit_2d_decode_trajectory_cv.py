...
# fit
def fit_2d_decode_trajectory_cv(
    epoch_idx,
    n_folds,
    movement="True",
    position_std=4.0,
        ...
        for fold_idx in range(n_folds):
            ...
            for trajectory_type in trajectory_types:
                ...
                for traj_start_time, traj_stop_time in trajectory_times[epoch][
                    ...
                    if movement:
                        inds_to_keep = exceeds_speed & within_trajectory & outside_fold
                    else:
                        inds_to_keep = within_trajectory & outside_fold
                    if inds_to_keep.sum() == 0:
                        continue
                    trajectory_position.append(position_interp[inds_to_keep])
                    trajectory_spike_indicator.append(spike_indicator[inds_to_keep])

                trajectory_position = np.concatenate(trajectory_position, axis=0)
                trajectory_spike_indicator = np.concatenate(
                    trajectory_spike_indicator, axis=0
                )
