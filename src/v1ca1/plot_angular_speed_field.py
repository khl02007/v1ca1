...
def load_classifier_dict(
    use_half="all",
    movement=True,
    headdir_speed_std=1.0,
    discrete_var="switching",
    headdir_speed_bin_size=1.0,
    movement_var=4.0,
):
...
def plot_angular_speed_field(
    headdir_speed_std=1.0,
):

    fig_save_path = analysis_path / "figs" / "angular_speed_field"
    fig_save_path.mkdir(parents=True, exist_ok=True)

    classifier_dict = load_classifier_dict(headdir_speed_std=headdir_speed_std)
    ...
    for region in regions:
        ...
        for unit_id in sorting[region].get_unit_ids():
            ...
            for trajectory_type in trajectory_types:
                ...
                for epoch_idx, epoch in enumerate(run_epoch_list):
                    ...
                    for traj_start_time, traj_end_time in trajectory_times[epoch][
                        ...
                        position_list.append(
                            f_head_direction_speed(spike_times_trajectory)
                        )
                    ...
                    ax[ax_row_offset + epoch_idx, ax_col_offset].set_xlim([0, 30])
            ...
            ax[-1, -1].set_xlabel("Angular speed (deg / s)")
