...
classifier_save_dir = analysis_path / "classifier_headdir"
...
def load_classifiers(
    ...
    n_folds: int,
    movement: str = "True",
    headdir_std: float = 4.0,
    discrete_var: str = "switching",
    headdir_bin_size: float = 1.0,
    movement_var: float = 4.0,
):
    epoch = run_epoch_list[epoch_idx]

    classifier_save_dir = analysis_path / "classifier_headdir_cv"
    ...
    for region in regions:
        ...
        for fold_idx in range(n_folds):
            classifier_save_path = classifier_save_dir / (
                ...
                f"_headdir_{fold_idx}_of_{n_folds}_folds"
                f"_movement_{movement}"
                f"_fit_headdir_std_{headdir_std}"
                f"_discrete_var_{discrete_var}"
                f"_headdir_bin_size_{headdir_bin_size}"
...
def compute_rate_function_headdir_cv(
    epoch_idx: int,
    n_folds: int,
    movement: str = "True",
    headdir_std: float = 4.0,
    ...
    rate_function_save_dir = analysis_path / "rate_function_headdir_cv"
    rate_function_save_dir.mkdir(parents=True, exist_ok=True)

    log_likelihood_save_dir = analysis_path / "log_likelihood_headdir_cv"
    ...
    classifier_dict = load_classifiers(
        ...
        n_folds=n_folds,
        movement=movement,
        headdir_std=headdir_std,
        discrete_var=discrete_var,
        headdir_bin_size=headdir_bin_size,
        movement_var=movement_var,
    )

    rate_function_save_path = rate_function_save_dir / (
        "rate_function_headdir"
        f"_{epoch}"
        f"_n_folds_{n_folds}"
        f"_movement_{movement}"
        f"_headdir_std_{headdir_std}"
        f"_discrete_var_{discrete_var}"
        f"_headdir_bin_size_{headdir_bin_size}"
    ...
    print(
        "Computing rate function by trajectory from head direction fields "
        f"{animal_name} {date} "
        f"epoch {epoch}, "
        f"n_folds {n_folds}, "
        f"movement {movement}, "
        f"headdir_std {headdir_std}, "
        f"discrete_var {discrete_var}, "
        f"headdir_bin_size {headdir_bin_size}, "
    ...
    for region in regions:
        ...
        f_head_direction_unwrapped = scipy.interpolate.interp1d(
            t_position,
            head_direction_deg_unwrapped,
            axis=0,
            bounds_error=False,
            kind="linear",
        )
        head_direction_unwrapped_interp = f_head_direction_unwrapped(time)
        ...
        for unit_id in sorting[region].get_unit_ids():
            ...
            for trajectory_type in trajectory_types:
                ...
                for fold_idx in range(n_folds):
                    ...
                    headdir_field = classifier.place_fields_[("", 0)][
                        :, unit_id
                    ].to_numpy()
                    ...
                    for traj_start_time, traj_stop_time in trajectory_times[epoch][
                        ...
                        head_direction_wrapped_interp = (
                            head_direction_unwrapped_interp[inds_to_keep] + 180
                        ) % (2 * 180) - 180

                        r = f_rate(head_direction_wrapped_interp)
                        nans = np.isnan(r)
                        if np.any(nans):
                            good = ~nans
                            if good.sum() == 0:
...
def main():
    ...
    compute_rate_function_headdir_cv(
        epoch_idx=args.epoch_idx,
        n_folds=args.n_folds,
        movement=args.movement,
        headdir_std=args.headdir_std,
        discrete_var=args.discrete_var,
        headdir_bin_size=args.headdir_bin_size,
